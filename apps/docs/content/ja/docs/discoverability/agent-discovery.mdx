...
エントリ識別子はあなたのドメインをアンカーとするため、カタログには [`deployment.site`](/docs/deployment) が必要です — 設定されていない場合は何も出力されません。デフォルトで有効で、`ai.catalog: false` で無効にできます。生成されるクエリは、サイトタイトルと各エントリ自身の説明から導出されます。あるエントリに独自のクエリを書くには、識別子の末尾（`<namespace>:<name>`）をキーにしてください:
