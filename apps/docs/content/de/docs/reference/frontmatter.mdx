...
`hidden` entfernt die Seite aus der Sidebar und aus der Zurück-/Weiter-Navigation. Auf der `index`-Seite eines Ordners entfernt es nur die Zeile der Seite selbst: Die Gruppenzeile verlinkt weiterhin auf die Seite, und Zurück-/Weiter-Links führen weiterhin durch sie hindurch.
...
Schemas werden über die [Standard-Schema](https://standardschema.dev)-Schnittstelle akzeptiert, sodass Zod (in der Version, die dein Projekt installiert), Valibot und ArkType allesamt funktionieren. Jeder deklarierte Schlüssel wird auf jeder Seite validiert — auch fehlende —, sodass ein erforderliches Schema den Schlüssel site-weit erzwingt; markiere ihn mit `.optional()`, um nur dort zu validieren, wo er vorhanden ist. Alle übrigen Schlüssel bleiben streng validiert, und eingebaute Felder können nicht neu deklariert werden.
