...
Para enviar cabeçalhos de requisição estáticos em toda chamada — um cabeçalho que identifica quem chama num backend compartilhado, por exemplo, para que a observabilidade ou a limitação de taxa dele consiga distinguir a sua documentação do restante do tráfego — defina `headers`. Funciona em todos os backends, inclusive no gateway:
...
As chaves são lidas através do [`getSecret()`](https://docs.astro.build/en/guides/environment-variables/#retrieving-secrets-programmatically) do Astro, então cada adaptador as fornece do seu próprio jeito: variáveis de ambiente no Node, na Vercel e no Netlify, e os [bindings](https://docs.astro.build/en/guides/integrations-guide/cloudflare/#environment-variables-and-secrets) do Worker no Cloudflare. Ativar o Ask AI também liga o React para a ilha na página — veja [Personalização](/docs/configuration/customization#interactive-islands).

## Raciocínio [#reasoning]

Modelos de raciocínio pensam antes de responder, e o quanto eles fazem isso por padrão varia de modelo para modelo. Para perguntas e respostas fundamentadas na documentação, os trechos recuperados carregam a resposta, então a maior parte desse pensamento é latência que o leitor aguarda. `reasoning` define o quanto o modelo raciocina: `"none"`, `"minimal"`, `"low"`, `"medium"`, `"high"` ou `"xhigh"`:
...
O valor é enviado como o controle de esforço de raciocínio do próprio backend. Pelo gateway ele viaja como a [opção `reasoning` do AI SDK](https://ai-sdk.dev/docs/ai-sdk-core/reasoning), que o gateway mapeia para a configuração do modelo — o `reasoning_effort` da OpenAI, por exemplo. No OpenRouter é enviado como `reasoning.effort`, e no LLMGateway ou num endpoint `openai-compatible` personalizado como `reasoning_effort` na requisição, então o endpoint precisa aceitar esse parâmetro. O modelo precisa suportar o nível que você escolher: a OpenAI rejeita um nível que o modelo não oferece (`"none"` e `"xhigh"` existem apenas em alguns), então confira a documentação do modelo antes de definir um. O Inkeep roda o próprio pipeline de QA e não tem controle de raciocínio, então definir `reasoning` com aquele backend é um erro de configuração. Deixe sem definir para manter o padrão do modelo. Assim como o [tamanho da recuperação](#retrieval-size), isso troca abrangência por tempo até o primeiro token, e as respostas continuam fundamentadas de qualquer forma.
