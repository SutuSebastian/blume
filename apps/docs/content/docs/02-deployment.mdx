...
Blume keeps two caches a build can reuse. Astro's and Vite's caches live under `.blume/.cache/` (the content store and image transforms among them). Rendered [OG cards](/discoverability/open-graph#card-cache) live in `node_modules/.cache/blume/og`, the conventional build-cache directory that Netlify, Vercel, and Cloudflare persist between deploys, so only the cards whose page title, description, or branding changed are re-rendered. On a CI runner you manage yourself, cache both directories between runs to get the same effect.
